 ...
 */
export function generateNarrative(results, t, archetypeLabels, logicLabels) {
    ...
    const dominantLogic = logicSorted[0][0];
    let paragraph = t(`organization.narrative.logic_${dominantLogic}`);
    ...
    let archSentence = t('organization.narrative.arch_focus')
        .replace('{arch1}', topArchLabels[0])
        .replace('{arch2}', topArchLabels[1]);

    paragraph += " " + archSentence;
    ...
    if (logicMarginals.SEP >= HIGH && logicMarginals.HYB < LOW && (diagnostics.silo_risk || 0) >= HIGH) {
        paragraph += " " + t('organization.narrative.risk_note').replace('{risk}', t('organization.riskSilo').toLowerCase());
    } else if (logicMarginals.INT >= HIGH && logicMarginals.HYB < LOW && (diagnostics.fragmentation_risk || 0) >= HIGH) {
        paragraph += " " + t('organization.narrative.risk_note').replace('{risk}', t('organization.riskFragmentation').toLowerCase());
    }
    ...
    if (lowestDiag && lowestDiag[1] < LOW) {
        const diagKey = `organization.diagnosticPrompts.${lowestDiag[0]}`;
        paragraph += " " + t('organization.narrative.diag_note').replace('{diag}', t(diagKey).replace('Fundera på: ', '').replace('Consider: ', '').toLowerCase());
    }
