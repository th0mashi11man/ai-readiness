 ...
 */
export function generateIndividualNarrative(results, t, domainLabels, narratives, locale) {
    const pct = (results.totalCorrect / results.totalItems) * 100;

    // Use external narratives for overview if available, fallback to t()
    let overview = "";
    ...
    // Find domains that need attention (score < 60%) - Lowered from 70%
    ...
    if (lowDomains.length > 0) {
        lowDomains.forEach(id => {
            const domainText = narratives?.individual?.[`${id}_low`]?.[locale];
            if (domainText) {
                feedbackParts.push(domainText);
            } else {
