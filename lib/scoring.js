...
export function generateNarrative(results, t, archetypeLabels, logicLabels, narratives, locale) {
    ...
    let archetypeIntro = "";
    if (spread < 15) {
        archetypeIntro = narratives?.organization?.archetype_profiles?.balanced?.[locale] || "";
    } else if ((archSorted[0][1] - archSorted[1][1]) > 15) {
        const base = narratives?.organization?.archetype_profiles?.focused_single?.[locale] || "";
        archetypeIntro = base.replace('{arch1}', topArchLabels[0]);
    } else {
        const base = narratives?.organization?.archetype_profiles?.focused_dual?.[locale] || "";
        archetypeIntro = base
    ...
    const archetypeItems = ARCHETYPE_IDS.map((archId, i) => {
        ...
        const text = narratives?.organization?.archetypes?.[archId]?.[isUnderrepresented ? "low" : "high"]?.[locale] || "";
    ...
    if (logicMarginals.SEP >= HIGH && logicMarginals.HYB < LOW && (diagnostics.silo_risk || 0) >= HIGH) {
        const baseRisk = narratives?.organization?.risks?.general_note?.[locale] || "";
    ...
    if (lowestDiag && lowestDiag[1] < LOW) {
        diagText = narratives?.organization?.diagnostics?.[lowestDiag[0]]?.[locale] || "";
    }
