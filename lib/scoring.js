...
export function generateNarrative(archetypeMarginals, logicMarginals, diagnostics, t) {
    const narrative = { leadingArchetypes: [], dominantLogic: null, risks: [], recommendations: [] };

    // Leading archetypes (top 1-2)
    ...
    // Recommendations (show when diagnostic is low < 50)
    if ((diagnostics.boundary_spanning || 0) < LOW) {
        narrative.recommendations.push(t('organization.recoBoundaryRoles'));
    }
    if ((diagnostics.data_readiness || 0) < LOW) {
        narrative.recommendations.push(t('organization.recoDataReadiness'));
    }
    if ((diagnostics.it_integration || 0) < LOW) {
        narrative.recommendations.push(t('organization.recoITIntegration'));
    }
    if ((diagnostics.partnerships || 0) < LOW) {
        narrative.recommendations.push(t('organization.recoPartnerships'));
    }
