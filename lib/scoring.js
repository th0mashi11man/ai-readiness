 ...
 * Gap levels should match the whole-number values shown in the UI, so we
 * classify against the rounded current score rather than the raw fractional average.
...
export function getGapLevel(priority, currentAverage) {
    const roundedCurrent = Math.round(currentAverage);
    const gap = priority - roundedCurrent;
