...
export function generateNarrative(results, t, archetypeLabels, logicLabels) {
    ...
    const logicText = t(`organization.narrative.logic_${dominantLogic}`);
    ...
    const archetypeText = t('organization.narrative.arch_focus')
    ...
    if (logicMarginals.SEP >= HIGH && logicMarginals.HYB < LOW && (diagnostics.silo_risk || 0) >= HIGH) {
        riskText = t('organization.narrative.risk_note').replace('{risk}', t('organization.riskSilo').toLowerCase());
    } else if (logicMarginals.INT >= HIGH && logicMarginals.HYB < LOW && (diagnostics.fragmentation_risk || 0) >= HIGH) {
        riskText = t('organization.narrative.risk_note').replace('{risk}', t('organization.riskFragmentation').toLowerCase());
    }
    ...
    if (lowestDiag && lowestDiag[1] < LOW) {
        ...
        diagText = t('organization.narrative.diag_note').replace('{diag}', t(diagKey).replace('Fundera på: ', '').replace('Consider: ', '').toLowerCase());
    }
